...
const adapter: VenueAdapter = new GrypsOrbsAdapter({
  solverBaseUrl:
    args.get('solver-base') ??
  ...
  async ({ category, rfqOnly, limit }) => {
    ...
    if (category) {
      ...
      markets = markets.filter((m) =>
        m.categories.some((x) => x.toLowerCase() === c),
      )
    }
    if (rfqOnly) markets = markets.filter((m) => m.isValid && m.rfqAllowed)
      ...
      content: [
        {
          type: 'text' as const,
          text: JSON.stringify(
            { total, venue: adapter.venueId, markets },
            null,
            1,
          ),
        },
...
server.tool(
  ...
  { symbol: z.string().describe('e.g. BTC, XAU, SEIUSDT') },
  async ({ symbol }) => {
...
server.tool(
  ...
  async () => {
    ...
    return {
      content: [{ type: 'text' as const, text: JSON.stringify(oi, null, 1) }],
    }
  },
)
...
server.tool(
  ...
  async () => {
    ...
    return {
      content: [{ type: 'text' as const, text: JSON.stringify(hb, null, 1) }],
    }
  },
)
...
server.tool(
  ...
  async ({ venueId, severity }) => {
    const files = readdirSync(QUIRKS_DIR).filter((f: string) => f.endsWith('.json'))
    const venues = files
      .map((f: string) => JSON.parse(readFileSync(join(QUIRKS_DIR, f), 'utf8')))
      .filter((v: { venue_id: string }) => !venueId || v.venue_id === venueId)
      .map((v: { quirks: Array<{ severity: string }> }) => ({
        ...v,
        quirks: v.quirks.filter(
          (q: { severity: string }) => !severity || q.severity === severity,
        ),
      }))
    return {
      content: [{ type: 'text' as const, text: JSON.stringify(venues, null, 1) }],
    }
  },
)
