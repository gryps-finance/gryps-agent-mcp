...
export const DEFAULT_POLYGON_ROLLUP: PolygonRollupConfig = {
...
export class PolygonRollupAdapter {
  ...
  constructor(private readonly config: PolygonRollupConfig = DEFAULT_POLYGON_ROLLUP) {}

  async heartbeat(): Promise<RollupHeartbeat> {
    const { rpcUrl, proxyAddress, usdcAddress } = this.config
    const call = (to: string, data: string) =>
      rpc(rpcUrl, 'eth_call', [{ to, data }, 'latest'])
    ...
    const [root, seqHex, blockHex, balHex, implRaw] = await Promise.all([
      ...
      call(
        usdcAddress,
        SEL_BALANCE_OF + proxyAddress.slice(2).toLowerCase().padStart(64, '0'),
      ),
